...
INLINE int handle_v4(
    ...
    if (!force_fakeip && port == 53U && config->dns_mode == SB_EBPF_DNS_MODE_OFF) return 1;
    if (connected_udp) {
    ...
    if (!connect_hook) {
        ...
        if (cached == FLOW_CACHE_PROXY ||
            (!force_fakeip && !intercept_dns && cached == FLOW_CACHE_BYPASS)) return 1;
    }
    if (!force_fakeip && !force_dns && uid_bypassed(config)) return 1;
    if (!force_fakeip && !intercept_dns) {
