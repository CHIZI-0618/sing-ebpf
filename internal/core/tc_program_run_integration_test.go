...
func TestFakeIPPolicyPrecedenceIntegration(t *testing.T) {
	requireEBPFIntegration(t, "verify FakeIP policy precedence in every TC data plane")
	policy, err := CompilePolicy(PolicyConfig{
		EnableTCP:           true,
		...
		SharedBypassPrivate: true,
		FakeIPIPv4:          netip.MustParsePrefix("198.18.0.0/15"),
		FakeIPIPv6:          netip.MustParsePrefix("fd00:198:18::/48"),
	...
	t.Run("socket_assign", func(t *testing.T) {
		...
		for name, packet := range fakeIPPolicyPrecedencePackets() {
			t.Run(name, func(t *testing.T) {
				action, _ := runTCProgram(t, program, packet)
				if action != testTCActShot {
					t.Fatalf("FakeIP did not override DNS, source, port, private, and CIDR bypass: action=%d", action)
				}
			})
		}
	})
		...
		for name, packet := range fakeIPPolicyPrecedencePackets() {
			t.Run(name, func(t *testing.T) {
				...
				if bytes.Equal(output, packet) {
					t.Fatal("FakeIP packet was not rewritten after matching lower-priority bypass policies")
				}
			})
		}
	})
...
func fakeIPPolicyPrecedencePackets() map[string][]byte {
