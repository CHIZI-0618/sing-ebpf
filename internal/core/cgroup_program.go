...
func (b *CgroupBackend) loadPrograms(listenerPort uint16) error {
	...
	for _, program := range b.runtime.programs {
		if program != nil {
			return eBPFOperationError("load eBPF cgroup programs", unix.EALREADY)
		}
	}
	if listenerPort == 0 {
	...
	if err != nil {
		b.listenerPort = 0
		return eBPFBackendOperationError("load eBPF cgroup programs", verifierErrorStage(err), err)
	}
...
func (b *CgroupBackend) updateCgroupControl(listenerPort uint16) error {
	...
	if b.forceInterceptIPv4.IsValid() {
		control.ForceInterceptIPv4Prefix = b.forceInterceptIPv4.Addr().As4()
		control.ForceInterceptIPv4Mask = prefixMask4(b.forceInterceptIPv4.Bits())
	}
	if b.forceInterceptIPv6.IsValid() {
		control.ForceInterceptIPv6Prefix = b.forceInterceptIPv6.Addr().As16()
		control.ForceInterceptIPv6Mask = prefixMask16(b.forceInterceptIPv6.Bits())
	}
	key := uint32(0)
