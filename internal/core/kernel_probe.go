...
func probeCommonCapabilities(report *KernelProbeReport, memlockErr error, plan kernelProbePlan) {
	...
	if needSelfBypass {
		...
		probeProgramType(report, "local", KernelProbePerformance, CiliumEBPF.CGroupSock,
			"Registers and releases consumer socket cookies for self-bypass.")
...
// probeICMPEchoReplyCapabilities checks the helpers native/icmp_echo_reply.bpf.c
...
// (KernelProbeOptions.ICMPEchoReply), which mirrors when prepareTC would
// actually load this object.
func probeICMPEchoReplyCapabilities(report *KernelProbeReport) {
	const scope = "icmp_echo_reply"
	report.Add(KernelProbePass, scope, KernelProbeRequired, "icmp_echo_reply reply facilities",
		"Answers ForceIntercept-destined ICMP Echo Requests in place on the same TC classifiers local/shared interception already attaches.")
...
func probeActivePrograms() ([]KernelProbeProgram, error) {
	...
	for {
		...
		if !isSingEBPFProgramName(info.Name) {
			continue
		}
